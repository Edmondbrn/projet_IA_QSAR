...
# ------------------------------------------------------------------------------------------------------
# Configuration des couches de neurones
# ------------------------------------------------------------------------------------------------------
...
for i, T in enumerate(total_neurones):
    ...
    qu.run_nested_cv_MLP(data, LC50, param_distributions, f"MLP_{T}", threshold_strict=0.13, threshold_soft=0.2)

input("Optimisation terminée, vueillez retourner sur le notebook 'def_AD_opti_param.ipynb' pour la suite")
quit() # a commenter si on veut la suite (uniquement si les données ont été générées)

# ------------------------------------------------------------------------------------------------------
# Evaluation des modèles (cross validation interne)
# ------------------------------------------------------------------------------------------------------
...
for model in best_models.iterrows(): # parcours des modèles
    ...
    for i in range(5): # on boucle 5 fois pour la cross-validation
        kf = KFold(n_splits=5, shuffle=True, random_state=42+i) # on divise les données en 5, 
